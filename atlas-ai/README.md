...
The frontend lives primarily in `src/App.tsx`, with typed Tauri command wrappers
in `src/shared/api/tauri.ts`. The backend entry point is `src-tauri/src/lib.rs`;
newer backend slices are split across `domain`, `app`, and `infra` modules for
models, jobs, search, diagnostics, benchmarks, summaries, memories, knowledge,
context assembly, and tools.

See `docs/architecture.md` for the current architecture notes.
